...
public class EmpresaService implements EmpresaInterface { // Se genera superclase de Inteface para que funcione
    ...
    // VER EMPRESAS
    @Override // Inyeccion, oblica a crear una interface diferente al repositorio, sirve para
              // enviar una peticion al repositorio
    ...
    public String actualizarEmpresa(Empresa emp, Integer id) throws Exception { /*
                                                                                 * Recibimos los parametros del
                                                                                 * controlador
                                                                                 */
        try {
            Empresa g = empresaGetId(id); /*
                                           * Se busca el id en el metodo de la linea 26, si encuentra el id
                                           * entonces guarda todos los datos de esa empresa en la variable g, en las
                                           * siguientes lineas se modifican los
                                           * datos
                                           */
            g.setId(id); // Se modifica id
            g.setNombre(emp.getNombre()); /*
                                           * El emp viene del parametro y el getnombre es el metodo para mostrar o
                                           * colocar
                                           * el nombre de la empresa que envia el usuario y se cambia el nombre al mismo
                                           * tiempo
                                           */
            g.setNit(emp.getNit());
            ...
            repoEmpresa.save(g); // Finalmente se guarda lo que se modifico en la variable g y quedan
